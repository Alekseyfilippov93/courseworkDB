...
from config import config  # Чтение параметров из .env
...
def insert_data_to_tables(conn: psycopg2.connect, companies_data: List[Dict], vacancies_data: List[Dict]) -> None:
    ...
    with conn.cursor() as cur:
        ...
        for vacancy in vacancies_data:
            ...
            try:
                ...
                print(f"[DB Error] Ошибка вставки вакансии {vacancy.get('name', 'N/A')}. Пропуск. {e}")

    conn.commit()
    print(f"[DB] Вставлено {len(companies_data)} компаний и {len(vacancies_data)} вакансий.")